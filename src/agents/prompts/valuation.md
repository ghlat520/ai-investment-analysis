...
```
公司是否有多条差异化业务线（增速/毛利率显著不同）？
├─ 是 且有YAML精细预测 → 分业务线前瞻盈利模型为主（代码已算好净利润/EPS/目标价）
│   → 你的任务：验证增速假设合理性、识别风险、交叉验证
├─ 是 且有API分业务数据 → 基于历史趋势+一致预期推算分业务前景
│   → 逐业务线分析增速和毛利率趋势，推算未来盈利
├─ 否 → 继续以下常规决策树
│
...
- 持续高股息（>3%）→ 类债券估值下限
- 股息率递增 → 管理层对盈利信心
- 无分红 → 关注留存收益用途（研发/扩产 vs 无盈利）

### 分业务线前瞻数据
{segment_forecast_context}
...
#### 四、分业务线前瞻估值
**A. 有分业务数据时：**
- 逐业务线分析：当前营收、毛利率、增速趋势
- 各业务线角色定位：增长引擎/现金牛/战略培育/拖累
- 分业务加权PE推导（高增长半导体40x vs 传统业务10x等）
- 多年期净利润预测 → EPS → 目标价（如YAML已算好则验证合理性）
- 与整体PE百分位交叉验证：前瞻估值是否与当前市场定价一致？

**B. 无分业务数据时（必须输出降级警告）：**
- 在 `precision_warnings` 数组中添加一条，说明：
  1. 缺失了什么数据（分业务营收构成/YAML精细模型/券商分业务预测）
  2. 估值精度因此受到什么影响（如：无法区分高增长业务线 vs 拖累业务线，只能用整体PE/PEG粗估，目标价置信度下降）
  3. 投资者应关注什么风险（如：公司若有多条差异化业务线，整体PE可能严重高估或低估某一业务板块的价值）
- 在 reasoning 中用"⚠️"标注此降级影响
- 不要跳过此步，而是明确告知"因缺少分业务数据，本次估值采用整体法，精度有限"
...
#### 七、主模型计算（展示完整推导过程）
- 如果上方有YAML精细预测的目标价表格：直接引用代码计算结果作为target_prices，不要重新计算
- 如果没有YAML数据：公式 → 参数取值及来源 → 代入计算 → 目标价
...
{{
  ...
  "segment_valuation": {{
    ...
    "segment_roles": [{{"name": "业务线名", "role": "增长引擎/现金牛/战略培育/拖累", "comment": "一句话评价"}}],
    "weighted_pe": 0.0,
    "forward_target_price": 0.0,
    "assumption_risks": ["string, 增速假设的主要风险"],
    "cross_validation": "string, 前瞻估值与当前PE百分位的一致性分析"
  }},
  "trap_detection": {{
  ...
  "precision_warnings": [
    {{
      "type": "missing_segment_data | stale_data | missing_research | other",
      "severity": "high | medium | low",
      "message": "string, 面向投资者的一句话警告（中文）",
      "impact": "string, 对估值结论的具体影响",
      "suggestion": "string, 建议投资者如何应对"
    }}
  ]
}}
```
...
- precision_warnings 必须如实反映估值精度的限制。典型场景：
  - 无分业务数据 → severity=high，说明整体PE/PEG法无法区分差异化业务线价值
  - 数据滞后>5天 → severity=medium，说明行情可能已变化
  - 无券商研报覆盖 → severity=medium，缺少专业机构交叉验证
  - 数据齐全无降级 → precision_warnings 为空数组 []
